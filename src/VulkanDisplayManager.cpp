...
VulkanDisplayManager::VulkanDisplayManager(int argc, char **argv) : DisplayManager(argc, argv)
{
    std::cout<<"VulkanDisplayManager Created."<<std::endl;
...
VulkanDisplayManager::~VulkanDisplayManager()
{
...
unsigned int VulkanDisplayManager::LoadShaderProgram(std::string a_szVertexShader, std::string a_szFragmentShader)
{
...
std::string VulkanDisplayManager::LoadShader(std::string a_szShaderName)
{
	int piSize;
...
bool VulkanDisplayManager::CreateScreen(int argc, char **argv)
{
...
int VulkanDisplayManager::GetEmptyTextureNumber()
{
	//Get a pointer.
...
int VulkanDisplayManager::LoadTexture(std::string a_sName)
{
...
int VulkanDisplayManager::LoadTextureSDLSurface(SDL_Surface* a_pkSurface)
{
...
void VulkanDisplayManager::UpdateTextureSDLSurface(SDL_Surface* a_pkSurface, int a_iTextureNumber)
{
	// get the number of channels in the texture to check if there's an alpha channel and also determine the format of the image
	GLenum texture_format = 0;
	GLint nOfColours;
...
void VulkanDisplayManager::UnloadTexture(int a_iTextureNumber)
{
	for(unsigned int iDx = 0; iDx < m_astLoadedTextures.size(); iDx++)
	{
...
void VulkanDisplayManager::UnloadTexture(std::string a_szTextureFilename)
{
	for(unsigned int iDx = 0; iDx < m_astLoadedTextures.size(); iDx++)
	{
...
bool VulkanDisplayManager::Update(float a_fDeltaTime)
{
...
float VulkanDisplayManager::TransformToScreenSpaceX(double a_pkPosition)
{
...
float VulkanDisplayManager::TransformToScreenSpaceY(double a_pkPosition)
{
	a_pkPosition *= m_pkViewMatrix->GetTranslation().z;
...
float VulkanDisplayManager::HUDTransformToScreenSpaceX(double a_pkPosition)
{
	return(float)(a_pkPosition / (m_iXResolution / 2))- 1.0f;
}

float VulkanDisplayManager::HUDTransformToScreenSpaceY(double a_pkPosition)
{
	return(float)( 1.0f - (a_pkPosition / (m_iYResolution / 2)));
}

void VulkanDisplayManager::SetViewMatrix(Matrix* a_pkNewMatrix)
{
...
void VulkanDisplayManager::SetCameraDimensions(float a_fCameraTop, float a_fCameraRight)
{
	m_fCameraTop = a_fCameraTop; m_fCameraRight = a_fCameraRight;
}

void VulkanDisplayManager::SetShaderProgram(unsigned int a_uiShaderProgramNumber)
{
	glUseProgram(a_uiShaderProgramNumber);
	return;
}

bool VulkanDisplayManager::HUDDraw(Vertex* a_aLocations, int a_iSizeOfArray, Texture* a_pkTexture)
{
...
bool VulkanDisplayManager::Draw(Mesh* a_pkMesh, int a_iSizeOfArray, Texture* a_pkTexture)
{
	glBindTexture(GL_TEXTURE_2D, a_pkTexture->GetTextureNumber());
